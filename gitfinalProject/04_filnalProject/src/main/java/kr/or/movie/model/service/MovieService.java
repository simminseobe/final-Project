...
public class MovieService {
	...
	public ArrayList<MyMovie> selectOneMovieAll2(int memberNo) {
		...
		/*
		 * for(Movie movie : list) { int movieNum= movie.getMovieNo(); MovieFile
		 * movieFile = dao.selectMovieFile(movieNum); movie.setMainFile(movieFile);
		 * Review movieScoreAvg=dao.onlyWatchPointAvg(movieNum);
		 * movie.setMovieScoreAvg(movieScoreAvg); }
		 */
	...
	public ReviewPageData selectReviewList(int movieNo, int reqPage, int memberNo) {
		...
		// 이전버튼 생성
		if (reqPage != 1) {
			pageNavi += "<a href='/movieDetail.do?movieNo=" + movieNo + "&reqPage=" + (pageNo - 1)
					+ "&stat=1'>[이전]</a>";
		}
		// 페이지 숫자 생성
		for (int i = 0; i < pageNaviSize; i++) {
			if (pageNo == reqPage) {
				...
				pageNavi += "<a href='/movieDetail.do?movieNo=" + movieNo + "&reqPage=" + pageNo + "&stat=1'>" + pageNo
						+ "</a>";
			}
			pageNo++;
			if (pageNo > totalPage) {
	...
	public Movie selectTitleMovie(String movieTitle) {
		Movie movie = dao.selectTitleMovie(movieTitle);
